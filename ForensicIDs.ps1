...
function Restart-WmiServiceSafely {
    ...
    if ($script:WmiRestarted) {
        return
    }

    try {
        Write-Host "Restarting WMI (Winmgmt) service to ensure fresh data retrival..."
        ...
        Write-Host "WARNING: WMI restart failed." -ForegroundColor Yellow
...
"@
    ...
    Write-Host
    Write-Host "      User Authorization Required      " -ForegroundColor Red
    Write-Host
    ...
    Write-Host "This script will perform administrative operations for Hardware Identification." -ForegroundColor Red
    Write-Host
...
function Collect-BiosCategory {
    ...
    Collect-Info "BIOS Revision"      { Join-Values $CIM.BIOS.BIOSVersion " " }
...
function Collect-SystemCategory {
    ...
    Collect-Info "System Version"      { $CIM.Product.Version }
...
function Collect-MotherboardCategory {
    ...
    Collect-Info "Motherboard Version"      { $CIM.BaseBoard.Version }
...
function Collect-DiskCategory {
    Collect-Info "Disk Model"         { Join-Values $CIM.Disk.Model }
    Collect-Info "Disk Interface"     { Join-Values $CIM.Disk.InterfaceType }
    Collect-Info "Disk Media Type"    { Join-Values $CIM.Disk.MediaType }
    Collect-Info "Disk Serial Number" { Join-Values $CIM.Disk.SerialNumber }
}
    ...
    if ($result["Firmware Type"] -eq "UEFI") {
        ...
        try {
            ...
            if ($priv -match "SeSystemEnvironmentPrivilege\s+Enabled") {
                ...
                $result["UEFI Variable Access Privilege"] = "Missing (OS blocked from reading UEFI variables by policy)"
            }
        }
        catch {
...
# ========================================================================
# SECTION 21 - DEFINE OUTPUT PATHS
# ========================================================================
...
BIOS Revision: $($hardwareDetails['BIOS Revision'])
